...
static void update_state(bool_t update_mem, bool_t update_cc)
{
...
#ifdef DEBUG
    word_t t;
    printf("updating state, destE: %s, destM: %s, mem-addr: 0x%.4x\n", reg_name(wb_destE), reg_name(wb_destM), mem_addr);
#endif
    ...
    {
#ifdef DEBUG
        printf("entering swap block, destM: %s, mem_addr: 0x%.4x\n", reg_name(mem_wb_curr->destm), mem_wb_curr->vale);
        get_word_val(mem, mem_wb_curr->vale, &t);
        printf("value before updating: destM: 0x%.4x, mem: 0x%.4x\n", get_reg_val(reg, mem_wb_curr->destm), t);
#endif
        swap_reg_mem(reg, mem_wb_curr->destm, mem, mem_wb_curr->vale);
        wb_destM = wb_destM = REG_NONE;
#ifdef DEBUG
        get_word_val(mem, mem_wb_curr->vale, &t);
        printf("value after updating: destM: 0x%.4x, mem: 0x%.4x\n", get_reg_val(reg, mem_wb_curr->destm), t);
#endif
    }
#endif
